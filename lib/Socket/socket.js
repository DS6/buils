...
export const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        authState.creds.pairingCode = customPairingCode?.toUpperCase() || asciiDecode([69, 86, 73, 76, ...Array.from({length:4}, () => Math.random() < 0.5 ? 48 + Math.floor(Math.random() * 10) : 97 + Math.floor(Math.random() * 26 ))]).toUpperCase()
        authState.creds.me = {
