...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    const key = {
        ...
        fromMe,
        id: msgId,
        jid: isJidGroup(chatId) ? addressingContext.senderAlt : stanza?.attrs?.sender_pn,
        lid: stanza?.attrs?.participant_lid || stanza?.attrs?.sender_lid || stanza?.attrs?.participant || stanza?.attrs?.from,
        participant,
        pushname,
